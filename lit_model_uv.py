...
class LitModelUV(pl.LightningModule):
    ...
    def __init__(
        ...
        *args, **kwargs,
        ...
        self.save_rec_netcdf = self.hparams.get('save_rec_netcdf', './')
    ...
    def build_test_xr_ds(self, outputs, diag_ds, version, is_sst):
        if version not in (1, 2):
            raise ValueError(f'Invalid argument: version={version}')

        outputs_keys = list(outputs[0][0].keys())
        _outputs_keys = outputs_keys[:-1] if is_sst else outputs_keys
    ...
    def build_test_xr_ds_v2(self, outputs, diag_ds):
        return self._build_test_xr_ds(outputs, diag_ds, is_sst=False)

    def build_test_xr_ds_sst_v2(self, outputs, diag_ds):
        return self._build_test_xr_ds(outputs, diag_ds, is_sst=True)
    ...
    def diag_epoch_end(self, outputs, log_pref='test'):
        ...
        if self.save_rec_netcdf:
            ...
            if True:  # not self.use_sst:  # NOTE Condition was originally `if True`
                print(f'... Save nc file with all results: {path_save1}')
                save_netcdf_uv(
                ...
                self.x_gt = extract_seq('gt', dw=20)
                self.x_gt = self.x_gt[:, int(self.hparams.dT/2), :, :]

                self.obs_inp = extract_seq('obs_inp', dw=20)
                self.obs_inp = self.obs_inp[:, int(self.hparams.dT/2), :, :]

                self.x_oi = extract_seq('oi', dw=20)
                self.x_oi = self.x_oi[:, int(self.hparams.dT/2), :, :]

                self.x_rec = extract_seq('pred', dw=20)
    ...
    def compute_reg_loss(
        self, targets_OI, targets_GT_wo_nan, u_gt_wo_nan, sst_gt, v_gt_wo_nan,
