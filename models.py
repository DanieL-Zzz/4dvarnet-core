...
class Weight_Network(torch.nn.Module):
    def __init__(self, shape_data, nb_phi, dw, in_channels):
        ...
        self.n_phi = nb_phi
        # self._already_forwarded_once = False

    # def _initial_weight(self, x):
    #     """
    #     Return the initial weight (w_0 = 1 and w_k = 0).
    #     """
    #     _shape = (
    #         x.shape[0],
    #         self.shape_data[0],
    ...
    #     w0 = torch.ones(_shape)  # weight of phi_0
    #     wk = torch.zeros(_shape)  # weight of phi_k with k>0

    #     return torch.concat(
    #         [w0] + [wk for _ in range(self.n_phi-1)],
    #         dim=1,
    #     )
    ...
    def forward(self, x_in):
        ...
        # if not self._already_forwarded_once:
        #     self._already_forwarded_once = True
        #     return self._initial_weight(x_in)

        # return self._initial_weight(x_in) - x_out
        return x_out

def get_weight(index, this_one):
    factor = .9 if index == 0 else .1
    return factor * torch.ones_like(this_one)
